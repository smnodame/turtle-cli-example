...
import { getLanguage } from 'react-native-lang'

export const createNewResponse = async (appId, answers) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${appId}/responses`, {
		headers: {
...
export const submitChargeOmise = (amount, currency, token, keyHash) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/omise`, {
		headers: {
			'Content-Type': 'text/plain',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const requestProducts = (products) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/request-products`, {
		headers: {
			'Content-Type': 'text/plain',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const updateResponseAnswer = (slug, answer) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/responses/${slug}/answer`, {
		headers: {
...
export const addToCart = (slug, productId) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/responses/${slug}/products/${productId}`, {
		headers: {
			'Content-Type': 'text/plain',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const startSession = async (appId) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${appId}/visitor/session`, {
		headers: {
			'Content-Type': 'text/plain',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION,
			'X-Resolution': `${Dimensions.get('window').width}x${Dimensions.get('window').height}`,
...
export const updateResponseStatus = (slug, status) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/responses/${slug}/status`, {
		headers: {
			'Content-Type': 'text/plain',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const fetchDesign = () => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/design`, {
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const fetchApp = () => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}`, {
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const eCommerceConfig = () => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/ecommerce-config`, {
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const fetchAppDetail = (isPreview) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/detail?isPreview=${isPreview}`, {
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const fetchConfig = (appId) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${appId}/message`, {
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const fetchStartOver = () => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/start-over`, {
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const fetchCredentials = async () => {
	return fetch(`${conf.API_BASE_URL}/v1/credentials`, {
		headers: {
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
		},
...
export const webhook = (payload) => {
	return fetch(`${conf.API_BASE_URL}/v1/webhook`, {
		headers: {
			'Content-Type': 'text/plain',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const uploadBlob = async (url, progressCallback = () => {}, extension = 'png') => {
	const blob = await fetch(url).then((res) => res.blob())
	const bodyFormData = new FormData()
	bodyFormData.append('file', blob, `${uniqid()}.${extension}`)
	return futch(
		`${conf.API_BASE_URL}/v1/upload-file`,
		{
			method: 'POST',
			body: bodyFormData,
			headers: {
...
export const sendRequestSMTP = (payload) => {
	return fetch(`${conf.API_BASE_URL}/v1/send-smtp`, {
		method: 'POST',
		headers: {
			'Content-Type': 'text/plain',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
...
export const registerCollectorAccount = () => {
	return fetch(`${conf.API_BASE_URL}/v1/collector-registration?token=${conf.DISTRIBUTE_TOKEN}`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			'X-Version': conf.VERSION
...
export const fetchSpreadsheetData = (spreadsheetId, sheetId) => {
	...
	if (sheetId) {
		return fetch(
			`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/spreadsheets/${spreadsheetId}/sheets/${sheetId}/data`,
			{
				headers: {
					'Content-Type': 'application/json',
					Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
	...
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/spreadsheets/${spreadsheetId}/data`, {
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
...
export const fetchMessageConfig = () => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/${conf.MODE === 'FULL-PREVIEW' ? 'preview-message' : 'message'}`, {
		headers: {
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
		},
...
export const generateQrCodePayment = async (input, userID) => {
	return fetch(`${conf.API_BASE_URL}/v1/apps/${conf.APP_SLUG}/users/${userID}/payment/qr_code/image`, {
		headers: {
			'Content-Type': 'text/plain',
			Authorization: `Bearer ${conf.ACCESS_TOKEN}`,
			'X-Version': conf.VERSION
