...
function* runMessage() {
	while (true) {
		...
		try {
			...
			const variables = yield select(state => state.chat.variables)
			...
			if (mode === 'UNION-MESSAGE') {
			...
			} else if (mode === 'PRODUCT-CAROUSEL-TEMPLATE') {
				yield* addMessageToChat({
					question: question,
					system: true,
				})
			...
			} else if (mode === 'GENERATE-QR-CODE-PAYMENT') {
				yield* addMessageToChat({
					question: question,
					system: true,
				})
				...
				const args = {
					...
					variables,
					updateOrders: (value) => {
						store.dispatch(actions.orders(value))
					}
				}

				yield call(backends['CLEAR-CART'], args)
			...
			} else if (mode === 'HUMAN-TAKEOVER') {
				const roomId = yield select(selectors.getRoomId)
				yield call(backends['HUMAN-TAKEOVER'], {
					roomId,
				})
