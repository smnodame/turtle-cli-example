...
import { TextInput as Input, Text, TouchableOpacity, View, FlatList, Image } from 'react-native'
...
class QuickReply extends React.Component {
	...
	onSelectQuickReply = (value) => {
		...
		// generate answer and replace expression
		const message = generateMessage(question.answer || value, answers, answer)
		onSend(
			{
				text: message,
			},
		...
		return (
			<View>
                <FlatList
					...
					data={this.props.question.input.textinput.enum}
					renderItem={({ item }) => {
						return (
							<TouchableOpacity 
								onPress={() => this.onSelectQuickReply(item.label)}
								style={{ 
									...componentStyles.quickReplyButton,
									backgroundColor: customStyle.container.backgroundColor, 
								}}
							>
								<Image source={{ uri: item.image }} style={{ ...componentStyles.quickReplyImage }} />
								<Text style={{ color: customStyle.input.color }}>{item.label}</Text>
							</TouchableOpacity>
						)
					}}
...
const Container = connect(mapStateToProps, mapDispatchToProps)(QuickReply)
